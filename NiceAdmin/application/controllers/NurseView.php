...
class NurseView extends CI_Controller {
	...
	public function index()
	{
        $data['doctor'] = $this->doc_model->getAllDoctors();  
         if(isset($_POST['user'])){
            $doc = $_POST['user'];
            $data['events']  = $this->calendarmodel->calSpec($doc);
            die(json_encode($data['events']));
        }else{
            $data['events']  = $this->calendarmodel->calSpec("mal12"); 
        }
...
//        redirect(base_url('DoctorView'));
    ...
    public function add_doc_notes(){
     ...
     public function getSpecDocDetails(){
        $events = $this->calendarmodel->cal();
        if(isset($_POST['user'])){
            $doc = $_POST['user'];
            $events = $this->calendarmodel->calSpec($doc);
            echo $events;
        }
